"""
vigIA — Estágio 1 | Fase 2: Modelagem por município
Entrada:  ../dados/dataset_municipio.csv
Saída:    ../modelos/municipio_avaliacao.pkl
          ../resultados/resultados_municipio_fase2.csv
...
print("=" * 65)
print("  vigIA E1 — Fase 2: Modelagem por Município")
print("=" * 65)

print("\n[1/5] Carregando dataset...")
ds = pd.read_csv(os.path.join(DADOS, "dataset_municipio.csv"), parse_dates=["Data"])
...
mlflow.set_experiment("vigIA_E1_municipio")
...
def avaliar(nome, modelo, params, elapsed):
    ...
    m = {
        ...
        "te_prec": precision_score(y_te, pred_te),
...
search_rf = RandomizedSearchCV(
    ...
    param_rf, n_iter=20, cv=cv, scoring="roc_auc", n_jobs=-1, random_state=SEED, verbose=1
)
...
param_xgb = {
    ...
    "learning_rate": [0.01,0.05,0.1], "subsample": [0.8,1.0], "colsample_bytree": [0.8,1.0],
}
try:
...
t0 = time.time()
search_xgb = RandomizedSearchCV(base_xgb, param_xgb, n_iter=30, cv=cv, scoring="roc_auc",
...
print("\n  [3/3] LightGBM...")
param_lgbm = {
    "n_estimators": [100,200,300], "max_depth": [-1,10,20],
...
search_lgbm = RandomizedSearchCV(
    ...
    param_lgbm, n_iter=30, cv=cv, scoring="roc_auc", n_jobs=-1, random_state=SEED, verbose=1
)
...
df_res.to_csv(os.path.join(RESULTADOS, "resultados_municipio_fase2.csv"), index=False)
...
joblib.dump({"modelo": melhor_mod, "features": FEATURES, "nome": melhor_nome, "params": melhor_params},
            os.path.join(MODELOS, "municipio_avaliacao.pkl"))
...
ax.set_title("vigIA E1 — AUC-ROC por Modelo | Município")
ax.legend()
fig.tight_layout()
fig.savefig(os.path.join(GRAFICOS, "e1_comparacao_modelos.png"), dpi=150)
plt.close()

print(f"\n{'='*65}")
print(df_res[["Modelo","val_auc","te_auc","te_f1","te_prec","te_rec"]].to_string(index=False))
print(f"{'='*65}")
print("\n[OK] E1 Fase 2 concluída!")
